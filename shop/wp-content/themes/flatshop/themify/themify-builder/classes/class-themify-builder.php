...
if ( ! class_exists( 'Themify_Builder' ) ) {
	...
	class Themify_Builder {
		...
		function builder_write_panels( $meta_boxes ) {
			global $Themify_Builder_Layouts, $pagenow;

			if ( 'post-new.php' == $pagenow ) return $meta_boxes;
			...
			$exclude_types = array( $Themify_Builder_Layouts->layout->post_type_name, $Themify_Builder_Layouts->layout_part->post_type_name );
		...
		function get_directory_path( $context ) {
			return $this->directory_registry[$context];
		}
		...
		function get_modules( $select = 'all' ) {
			...
			foreach( $this->get_directory_path( 'modules' ) as $dir ) {
				foreach( glob( $dir . '/module-*.php' ) as $filename ) {
					$module_name = basename( $filename );
					$_modules[$module_name] = $filename;
				}
			}
		...
		function builder_admin_bar_menu( $wp_admin_bar ) {
			...
			if ( is_singular() || is_page() ) {
				...
				if ( ! is_singular( $Themify_Builder_Layouts->post_types ) || ! Themify_Builder_Model::is_prebuilt_layout( $post->ID ) ) {
		...
		function do_search( $where ){
			if( is_search() ) {
				global $wpdb, $Themify_Builder_Layouts;
				...
				$where .= " OR {$wpdb->posts}.ID IN (
						...
						AND {$wpdb->posts}.post_type NOT IN ( '{$Themify_Builder_Layouts->layout->post_type_name}', '{$Themify_Builder_Layouts->layout_part->post_type_name}' ))";
			}
			return $where;
		}
		...
		function render_row_styling( $builder_id, $row ) {
			$settings = isset( $row['styling'] ) ? $row['styling'] : '';
			...
			foreach( $class_fields as $field ) {
				if ( isset( $row['styling'][ $field ] ) && ! empty( $row['styling'][ $field ] ) ) {
					if ( 'animation_effect' == $field )
						array_push( $row_classes, 'wow' );
					array_push( $row_classes, $row['styling'][ $field ] );
				}
			}
			$output = PHP_EOL; // add line break
			ob_start();
