...
/***************************************************************************/	
	...
	function themify_page() {
		...
		<p class="top-save-btn"><a href="#" id="save-button" class="save-button"><?php _e('Save', 'themify'); ?></a></em></p>
		<div id="content">
...
function themify_compile_protected_meta_list() {
	...
	$themify_protected_meta = array();
	if( ! empty( $themify_write_panels ) ) : foreach( $themify_write_panels as $metabox ) :
		$themify_protected_meta[] = wp_list_pluck( $metabox['options'], 'name' );
	endforeach; endif;

	// flat the list array
	$themify_protected_meta= call_user_func_array( 'array_merge', $themify_protected_meta );
}
	...
	function themify_get_image( $args ) {
		...
		if ( isset( $src ) && $src != '' && $src ) {
			...
			if( ! isset($data['setting-img_settings_use']) || '' == $data['setting-img_settings_use'] || !$data['setting-img_settings_use'] ){
				...
				$timg = themify_do_img( null, $src, $width, $height, $crop );
...
function themify_has_styling_data() {
	...
	/* short circuit the themify_has_styling_data check */
	if( apply_filters( 'themify_show_styling_panel', false ) ) {
		return true;
	}

	$flag = 'themify_has_styling_custom_css';
	$change = get_option( $flag );
	if ( ! $change ) {
		$styling_data = array();
		foreach ( themify_get_data() as $key => $val ) {
		...
		if ( ! empty( $styling_data ) || ! empty( $custom_css ) ) {
			update_option( $flag, 'has_styling' );
			$change = 'has_styling';
		} else {
			update_option( $flag, 'no_styling' );
			$change = 'no_styling';
		}
	}
	return $change == 'has_styling';
}
